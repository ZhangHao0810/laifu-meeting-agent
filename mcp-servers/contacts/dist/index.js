...
server.registerTool("contacts_get_detail", {
...
server.registerTool("contacts_get_metadata_options", {
...
// --- Organization Tools ---
server.registerTool("contacts_search_departments", {
...
server.registerTool("contacts_get_department_detail", {
    ...
    return {
        ...
        structuredContent: { ...dept }
    };
});
server.registerTool("contacts_get_org_hierarchy", {
    ...
    if (direction === "up") {
        ...
        while (currentEmp) {
            ...
            if (!managerName || managerName === "None" || managerName === "无")
                break;
            const managerEmp = EMPLOYEES.find(e => e.name === managerName);
            if (!managerEmp || managerEmp.user_id === currentEmp.user_id)
                break;
            currentEmp = managerEmp;
            if (report_line.length > 15)
                break;
        }
    }
    else {
